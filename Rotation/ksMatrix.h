...
extern "C" {
	...
	/// x, y, z Specify the x, y and z coordinates of a vector, respectively(旋转轴向量，angle增大左手定则旋转)
	//
	...
	//// result returns identity matrix(单位矩阵)
	//
	...
	/// fovy Field of view y angle in degrees（视角）
	/// aspect Aspect ratio of screen（长宽比）
	/// nearZ Near plane distance（近平面距离）
