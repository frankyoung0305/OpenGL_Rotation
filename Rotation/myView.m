...
- (void)compileShaders {
    ...
    // 获取uniform入口
//    _varyColorSlot = glGetUniformLocation(programHandle, "VaryingColor");
//    _radianAngle = glGetUniformLocation(programHandle, "RadianAngle");
//    _transformMat = glGetUniformLocation(programHandle, "TransMat");
...
-(void)updateProjection
{
    ...
    //视角，长宽比，近平面距离，远平面距离
    ksPerspective(&_projectionMatrix, 60.0, aspect, 1.0f, 1000.0f);
...
- (void)setup {
    ...
    _hahaTexture = [self setupTexture:@"pic.png"];
...
//    float ration = (frames % 10)*0.1;
//    scale(&trans, ration);
    
//    rotateX(&trans, frames*0.01);
    
//    [self translate:&trans withX:0.1 withY:0.1 withZ:0.5];
    
//    glUniformMatrix4fv(_transformMat, 1, GL_TRUE, &trans.mat[0][0]);

    
//    float green = (frames++ & 255) / 255.f;
//    glUniform4f(_varyColorSlot, 0.0f, green, 0.0f, 1.0f);  //写入uniform
    ...
    glBindTexture(GL_TEXTURE_2D, _hahaTexture);
